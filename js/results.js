const map = L.map('map').setView([25.034030, 121.563900], 8);
...
info.update = function() {
    this._div.innerHTML = '<h4>2025 年罷免案投票結果</h4>點擊村里查看詳細資料';
};
...
async function loadVillageSummary() {
    try {
        const response = await fetch('data/cunli_summary.json');
...
async function loadRecallCases() {
    try {
        ...
        const response = await fetch('data/recall_cases.json');
...
async function loadRecallCasesFromVillages() {
    try {
        ...
        for (const villcode of sampleVillages) {
            try {
                const response = await fetch(`data/${villcode}.json`);
...
async function initializeMap() {
    try {
        ...
        const response = await fetch('data/20250620.json');
...
function showChart() {
    ...
    if (currentRecallCase) {
        ...
        chartTitle.textContent = '2025 年罷免案總體投票結果分析';
    }
